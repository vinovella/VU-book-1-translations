translate portuguese strings:

    # game/code/screens/others.rpy:8
    old "Save your game now"
    new "Salve seu jogo agora"

    # game/code/screens/others.rpy:10
    old "{u}Return to Main Menu{/u}"
    new "{u}Retornar ao Menu Principal{/u}"

    # game/code/screens/others.rpy:182
    old "Warning!"
    new "Aviso!"

    # game/code/screens/others.rpy:184
    old "This game is intended for a mature audience of 18 years or older."
    new "Esse jogo deve ser jogado por uma audiência adulta de 18 anos ou mais."

    # game/code/screens/others.rpy:185
    old "All characters portrayed are 18 years or older. No exceptions."
    new "Todos personagens mostrados têm 18 anos ou mais. Sem exceção."

    # game/code/screens/others.rpy:186
    old "Some of the contents depicted in this game may be offensive to some players. Viewer discretion is advised."
    new "Certos conteúdos nesse jogo podem ser ofensivos para alguns jogadores. Prossiga por sua própria conta."

    # game/code/screens/others.rpy:187
    old "The content explored should not be treated as an endorsement of particular fetishes, kinks, or patterns of behaviour."
    new "O conteúdo explorado não deve ser tratado como aprovação de específicos fetiches, taras, ou padrões de comportamento."

    # game/code/screens/others.rpy:189
    old "Enjoy!"
    new "Aproveite!"

    # game/code/screens/others.rpy:213
    old "Coming Soon"
    new "Em Breve"

translate portuguese strings:

    # game/code/screens/others.rpy:10
    old "We collect only anonymized data to better understand which paths, characters, scenes, and fetishes are most popular and to identify areas for improvement."
    new "We collect only anonymized data to better understand which paths, characters, scenes, and fetishes are most popular and to identify areas for improvement."

    # game/code/screens/others.rpy:11
    old "NO PERSONAL INFORMATION IS COLLECTED."
    new "NO PERSONAL INFORMATION IS COLLECTED."

    # game/code/screens/others.rpy:13
    old "Would you like to enable this data collection?"
    new "Would you like to enable this data collection?"

