translate turkish strings:

    # game/code/data/achievement_gallery.rpy:5
    old "01: Scratching the Surface"
    new "01: Yüzeyi Kazımak"

    # game/code/data/achievement_gallery.rpy:5
    old "Finish the game."
    new "Oyunu bitir."

    # game/code/data/achievement_gallery.rpy:5
    old "02: Dulled Euphoria"
    new "02: Körelmiş Coşku"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Iona's path."
    new "Iona'nın yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "03: Monika's Melons"
    new "03: Monika'nın Kavunları"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Monika's path."
    new "Monika'nın yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "04: Queen Bae"
    new "04: Kraliçe Bae"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Isa's path."
    new "Isa'nın yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "05: Down With the Bourgeoisie"
    new "05: Kahrolsun Burjuvazi"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Sam's path."
    new "Sam'in yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "06: Blood and Boners"
    new "06: Kan ve Kemikler"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Delphia's path."
    new "Delphia'nın yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "07: Bagging Bookworms"
    new "07: Kitap Kurtlarını Torbalamak"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Irene's path."
    new "Irene'nin yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "08: Innocence Irrelevant"
    new "08: Masumiyet İlgisiz"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Lumi's path."
    new "Lumi'nin yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "09: Strokes on Canvas"
    new "09: Tuval Üzerine Vuruşlar"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Ezra's path."
    new "Ezra'nın yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "10: Wine Mom"
    new "10: Şarap Anne"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Sally's path."
    new "Sally'nin yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "11: Fleur de Passion"
    new "11: Tutku Çiçeği"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Fleur's path."
    new "Fleur'un yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "12: Prescription Desires"
    new "12: Reçeteli Arzular"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Asami's path."
    new "Asami'nin yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "13: What Did the Fox Moan"
    new "13: Tilki Ne İnledi"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Foxy's path."
    new "Foxy'nin yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "14: Mighty Morgan"
    new "14: Kudretli Morgan"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Morgan's path."
    new "Morgan'ın yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "15: Feared Straight"
    new "15: Korkulan Düzlük"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Corinne's path."
    new "Corinne'in yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "16: Juggling Favors"
    new "16: Hokkabazlık İyilikleri"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Nirmala's path."
    new "Nirmala'nın yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "17: Lifelong Longing"
    new "17: Yaşam Boyu Özlem"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Josie's path."
    new "Josie'nin yolunu oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "18: And He Was Born"
    new "18: Ve O Doğdu"

    # game/code/data/achievement_gallery.rpy:5
    old "Select your name."
    new "Adınızı seçin."

    # game/code/data/achievement_gallery.rpy:5
    old "19: The True Beginning"
    new "19: Gerçek Başlangıç"

    # game/code/data/achievement_gallery.rpy:5
    old "Reach the end of the prologue."
    new "Önsözün sonuna ulaş."

    # game/code/data/achievement_gallery.rpy:5
    old "20: Saving Private Josie"
    new "20: Er Josie'yi Kurtarmak"

    # game/code/data/achievement_gallery.rpy:5
    old "Save Josie from drowning."
    new "Josie'yi boğulmaktan kurtar."

    # game/code/data/achievement_gallery.rpy:5
    old "21: On to the Tournament"
    new "21: Turnuvaya Doğru"

    # game/code/data/achievement_gallery.rpy:5
    old "Finished the Winner's Part 2."
    new "Kazananın 2. Bölümünü Bitirdi."

    # game/code/data/achievement_gallery.rpy:5
    old "22: On to the Forest"
    new "22: Ormana Doğru"

    # game/code/data/achievement_gallery.rpy:5
    old "Finished the Cult's Part 2."
    new "Tarikatın 2. Bölümünü Bitirdim."

    # game/code/data/achievement_gallery.rpy:5
    old "23: On to the Compound"
    new "23: Yerleşkeye Doğru"

    # game/code/data/achievement_gallery.rpy:5
    old "Finished the Syndicate's Part 2."
    new "Sendika'nın 2. Bölümünü Bitirdim."

    # game/code/data/achievement_gallery.rpy:5
    old "24: Injecting the L"
    new "24: L Enjeksiyonu"

    # game/code/data/achievement_gallery.rpy:5
    old "Lose the Winner's tournament."
    new "Kazananın turnuvasını kaybet."

    # game/code/data/achievement_gallery.rpy:5
    old "25: Heart Shaped Box"
    new "25: Kalp Şeklinde Kutu"

    # game/code/data/achievement_gallery.rpy:5
    old "Experience spiritual awakening with Nia."
    new "Nia ile ruhsal uyanışı deneyimleyin."

    # game/code/data/achievement_gallery.rpy:5
    old "26: Kablooey"
    new "26: Kablooey"

    # game/code/data/achievement_gallery.rpy:5
    old "See the Wasp's warehouse explosion."
    new "Wasp'ın depo patlamasını gör."

    # game/code/data/achievement_gallery.rpy:5
    old "27: Threesome Therapy"
    new "27: Üçlü Terapi"

    # game/code/data/achievement_gallery.rpy:5
    old "Have a threesome with Nia and Silvia."
    new "Nia ve Silvia ile üçlü yapın."

    # game/code/data/achievement_gallery.rpy:5
    old "28: Temptations in Ink"
    new "28: Mürekkepteki Ayartmalar"

    # game/code/data/achievement_gallery.rpy:5
    old "Have a threesome with Nia and Zemfira."
    new "Nia ve Zemfira ile üçlü yapın."

    # game/code/data/achievement_gallery.rpy:5
    old "29: Rebel Without a Ball"
    new "29: Topsuz Asi"

    # game/code/data/achievement_gallery.rpy:5
    old "Have a threesome with Zemfira and Silvia."
    new "Zemfira ve Silvia ile üçlü yapın."

    # game/code/data/achievement_gallery.rpy:5
    old "30: Hacker's Delight"
    new "30: Hacker'ın Lokumu"

    # game/code/data/achievement_gallery.rpy:5
    old "Click the Butcher's street background character."
    new "Kasap'ın sokak arka plan karakterine tıkla."

    # game/code/data/achievement_gallery.rpy:5
    old "31: Meat Lovers"
    new "31: Et Severler"

    # game/code/data/achievement_gallery.rpy:5
    old "Win Greg's minigame."
    new "Greg'in mini oyununu kazan."

    # game/code/data/achievement_gallery.rpy:5
    old "32: Top of the Class"
    new "32: Sınıfının En İyisi"

    # game/code/data/achievement_gallery.rpy:5
    old "Earn 18+ points with a group."
    new "Bir grupla 18+ puan kazan."

    # game/code/data/achievement_gallery.rpy:5
    old "33: Eskimo Sisters"
    new "33: Eskimo Kız Kardeşler"

    # game/code/data/achievement_gallery.rpy:5
    old "Have sex with Monika and kiss Olesya."
    new "Monika ile seks yap ve Olesya'yı öp."

    # game/code/data/achievement_gallery.rpy:5
    old "34: The Truth Will Free You"
    new "34: Gerçek Sizi Özgür Bırakacak"

    # game/code/data/achievement_gallery.rpy:5
    old "Kiss Phoebe and tell Fleur and Asami the truth."
    new "Phoebe'yi öp ve Fleur ile Asami'ye gerçeği söyle."

    # game/code/data/achievement_gallery.rpy:5
    old "35: Junior Detective"
    new "35: Küçük Dedektif"

    # game/code/data/achievement_gallery.rpy:5
    old "Unlock a character on the investigation board."
    new "Soruşturma tahtasındaki bir karakterin kilidini aç."

    # game/code/data/achievement_gallery.rpy:5
    old "36: Privates Investigator"
    new "36: Özel Müfettiş"

    # game/code/data/achievement_gallery.rpy:5
    old "Unlock 20 characters on the investigation board."
    new "Soruşturma panosundaki 20 karakterin kilidini aç."

    # game/code/data/achievement_gallery.rpy:5
    old "37: Master Sleuth"
    new "37: Usta Sleuth"

    # game/code/data/achievement_gallery.rpy:5
    old "Unlock 40 characters on the investigation board."
    new "Soruşturma panosundaki 40 karakterin kilidini aç."

    # game/code/data/achievement_gallery.rpy:5
    old "38: Passion Detective"
    new "38: Tutku Dedektifi"

    # game/code/data/achievement_gallery.rpy:5
    old "Have sex with 10 girls."
    new "10 kızla seks yap."

    # game/code/data/achievement_gallery.rpy:5
    old "39: Campus Casanova"
    new "39: Kazanova Kampüsü"

    # game/code/data/achievement_gallery.rpy:5
    old "Have sex with all the girls."
    new "Bütün kızlarla seks yap."

    # game/code/data/achievement_gallery.rpy:5
    old "40: Professor's Pet"
    new "40: Profesörün Evcil Hayvanı"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Diana and Ronja's paths."
    new "Diana ve Ronja'nın yollarını oyna."

    # game/code/data/achievement_gallery.rpy:5
    old "41: Fetish Locator"
    new "41: Fetiş Bulucu"

    # game/code/data/achievement_gallery.rpy:5
    old "Experience all fetishes at the party."
    new "Partideki tüm fetişleri deneyimle."

    # game/code/data/achievement_gallery.rpy:5
    old "42: Calm and Collected"
    new "42: Sakin ve Toparlanmış"

    # game/code/data/achievement_gallery.rpy:5
    old "Throw Brigitta into the pool."
    new "Brigitta'yı havuza at."

    # game/code/data/achievement_gallery.rpy:5
    old "43: Yellow Balling"
    new "43: Sarı Top"

    # game/code/data/achievement_gallery.rpy:5
    old "Get blue balled by Nia and Zemfira at the party."
    new "Partide Nia ve Zemfira tarafından mavi topa tutul."

    # game/code/data/achievement_gallery.rpy:5
    old "44: Escaping the Matrix"
    new "44: Matrix'ten Kaçış"

    # game/code/data/achievement_gallery.rpy:5
    old "Click the unclickable option in the nightmare."
    new "Kabustaki tıklanamayan seçeneğe tıkla."

    # game/code/data/achievement_gallery.rpy:5
    old "45: Behind the Scenes"
    new "45: Perde Arkası"

    # game/code/data/achievement_gallery.rpy:5
    old "Play postcredit scene."
    new "Postcredit sahnesini oynat."

