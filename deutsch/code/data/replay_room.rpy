translate deutsch strings:

    # game/code/data/replay_room.rpy:5
    old "C01: Opening Cutscene"
    new "C01: Eröffnungssequenz"

    # game/code/data/replay_room.rpy:5
    old "C02: Training Cutscene"
    new "C02: Trainings-Zwischensequenz"

    # game/code/data/replay_room.rpy:5
    old "C04: Phoebe Dream"
    new "C04: Phoebe Traum"

    # game/code/data/replay_room.rpy:5
    old "C04: Delphia Masturbating"
    new "C04: Delphia masturbiert"

    # game/code/data/replay_room.rpy:5
    old "C04: Painting Delphia"
    new "C04: Bemale Delphia"

    # game/code/data/replay_room.rpy:5
    old "C04: Locker Room Fling With Iona"
    new "C04: Umkleidekabinen-Affäre mit Iona"

    # game/code/data/replay_room.rpy:5
    old "You like partying."
    new "Du magst Partys."

    # game/code/data/replay_room.rpy:5
    old "C04: Favor From Nirmala"
    new "C04: Gefallen von Nirmala"

    # game/code/data/replay_room.rpy:5
    old "Redeem the favor."
    new "Den Gefallen einlösen."

    # game/code/data/replay_room.rpy:5
    old "C05: Josie Sleepwalk"
    new "C05: Josie schlafwandelnd"

    # game/code/data/replay_room.rpy:5
    old "C05: Private Lesson With Ronja"
    new "C05: Privatunterricht mit Ronja"

    # game/code/data/replay_room.rpy:5
    old "C06: Ronja Booty Call"
    new "C06: Ronja Sexdate"

    # game/code/data/replay_room.rpy:5
    old "Call Ronja."
    new "Ruf Ronja an."

    # game/code/data/replay_room.rpy:5
    old "C06: Free Bone From Delphia"
    new "C06: Freier Knochen von Delphia"

    # game/code/data/replay_room.rpy:5
    old "Ask the right questions."
    new "Stell die richtigen Fragen."

    # game/code/data/replay_room.rpy:5
    old "C06: Worshipping Sally"
    new "C06: Sally verehren"

    # game/code/data/replay_room.rpy:5
    old "Stay with Sally."
    new "Bleib bei Sally."

    # game/code/data/replay_room.rpy:5
    old "C06: Handjob From Fleur"
    new "C06: Handjob von Fleur"

    # game/code/data/replay_room.rpy:5
    old "Let Fleur continue."
    new "Lass Fleur weitermachen."

    # game/code/data/replay_room.rpy:5
    old "C06: Pool Game With Nirmala"
    new "C06: Poolspielchen mit Nirmala"

    # game/code/data/replay_room.rpy:5
    old "C06: Finish What Fleur Started"
    new "C06: Beende, was Fleur begonnen hat"

    # game/code/data/replay_room.rpy:5
    old "Go with Fleur."
    new "Geh mit Fleur."

    # game/code/data/replay_room.rpy:5
    old "C06: Help From Iona"
    new "C06: Hilfe von Iona"

    # game/code/data/replay_room.rpy:5
    old "Go with Iona."
    new "Geh mit Iona."

    # game/code/data/replay_room.rpy:5
    old "C07: Diana Jumps You"
    new "C07: Diana springt dich an"

    # game/code/data/replay_room.rpy:5
    old "Don't rebuff Diana."
    new "Weise Diana nicht zurück."

    # game/code/data/replay_room.rpy:5
    old "C07: Sally's Hidden Entrance"
    new "C07: Sallys versteckter Eingang"

    # game/code/data/replay_room.rpy:5
    old "Reward Sally."
    new "Belohne Sally."

    # game/code/data/replay_room.rpy:5
    old "C07: Bonding With Nia"
    new "C07: Bindung mit Nia"

    # game/code/data/replay_room.rpy:5
    old "C07: Getting Better Acquainted"
    new "C07: Besser kennenlernen"

    # game/code/data/replay_room.rpy:5
    old "C07: Finish What Delphia Started"
    new "C07: Beende, was Delphia begonnen hat"

    # game/code/data/replay_room.rpy:5
    old "Help Delphia."
    new "Hilf Delphia."

    # game/code/data/replay_room.rpy:5
    old "C07: Seduce Irene"
    new "C07: Irene verführen"

    # game/code/data/replay_room.rpy:5
    old "Go all the way."
    new "Geh den ganzen Weg."

    # game/code/data/replay_room.rpy:5
    old "C07: An Artist's Head"
    new "C07: Der Kopf eines Künstlers"

    # game/code/data/replay_room.rpy:5
    old "Investigate the artist."
    new "Recherchiere über den Künstler."

    # game/code/data/replay_room.rpy:5
    old "C07: Karaoke With Nirmala"
    new "C07: Karaoke mit Nirmala"

    # game/code/data/replay_room.rpy:5
    old "C07: Reward From Corinne"
    new "C07: Belohnung von Corinne"

    # game/code/data/replay_room.rpy:5
    old "Investigate Corinne."
    new "Untersuche Corinne."

    # game/code/data/replay_room.rpy:5
    old "C07: Fun Time With Iona"
    new "C07: Spaß mit Iona"

    # game/code/data/replay_room.rpy:5
    old "Stop Iona from serving."
    new "Verhinder, dass Iona ihren Dienst verrichtet."

    # game/code/data/replay_room.rpy:5
    old "C07: Skinny Dipping With Isa"
    new "C07: Nacktbaden mit Isa"

    # game/code/data/replay_room.rpy:5
    old "Investigate Isa."
    new "Untersuche Isa."

    # game/code/data/replay_room.rpy:5
    old "C07: Reward From Sam"
    new "C07: Belohnung von Sam"

    # game/code/data/replay_room.rpy:5
    old "Investigate Sam."
    new "Untersuche Sam."

    # game/code/data/replay_room.rpy:5
    old "C08: Ivy's Welcoming Party"
    new "C08: Ivy's Willkommensparty"

    # game/code/data/replay_room.rpy:5
    old "C08: Loving Lumi"
    new "C08: Liebliche Lumi"

    # game/code/data/replay_room.rpy:5
    old "Comfort Lumi."
    new "Tröste Lumi."

    # game/code/data/replay_room.rpy:5
    old "C08: Ritual In The Cave"
    new "C08: Ritual in der Höhle"

    # game/code/data/replay_room.rpy:5
    old "C08: Spiritual Awakening With Nia"
    new "C08: Spirituelles Erwachen mit Nia"

    # game/code/data/replay_room.rpy:5
    old "C08: Lumi's Punishment"
    new "C08: Lumis Strafe"

    # game/code/data/replay_room.rpy:5
    old "C08: Self-Help Group Orgy"
    new "C08: Selbsthilfegruppen-Orgie"

    # game/code/data/replay_room.rpy:5
    old "C08: Breeding Ivy"
    new "C08: Schwänger Ivy"

    # game/code/data/replay_room.rpy:5
    old "C08: Date with Zemfira"
    new "C08: Date mit Zemfira"

    # game/code/data/replay_room.rpy:5
    old "C08: Comforting Asami"
    new "C08: Tröste Asami"

    # game/code/data/replay_room.rpy:5
    old "Help out Asami."
    new "Hilf Asami."

    # game/code/data/replay_room.rpy:5
    old "C08: Hellfire Hellena"
    new "C08: Höllenfeuer Hellena"

    # game/code/data/replay_room.rpy:5
    old "You don't mind at all."
    new "Das macht dir überhaupt nichts aus."

    # game/code/data/replay_room.rpy:5
    old "C08: In The Boss's Bed"
    new "C08: Im Bett der Chefin"

    # game/code/data/replay_room.rpy:5
    old "C08: Taking Care Of Zemfira"
    new "C08: Sich um Zemfira kümmern"

    # game/code/data/replay_room.rpy:5
    old "C08: Licking Morgan's Wounds"
    new "C08: Morgans Wunden lecken"

    # game/code/data/replay_room.rpy:5
    old "Ask Morgan to stay."
    new "Bitte Morgan, zu bleiben."

    # game/code/data/replay_room.rpy:5
    old "C08: Extra Practice With Iona"
    new "C08: Zusätzliches Training mit Iona"

    # game/code/data/replay_room.rpy:5
    old "Encourage Iona."
    new "Ermutige Iona."

    # game/code/data/replay_room.rpy:5
    old "C08: Holding Moans With Monika"
    new "C08: Stöhne mit Monika"

    # game/code/data/replay_room.rpy:5
    old "Give into the temptations."
    new "Gib der Versuchung nach."

    # game/code/data/replay_room.rpy:5
    old "C08: Healed By The Enemy"
    new "C08: Vom Feind geheilt"

    # game/code/data/replay_room.rpy:5
    old "Seize the moment."
    new "Nutze den Moment."

    # game/code/data/replay_room.rpy:5
    old "C08: Comforting Silvia"
    new "C08: Silvia trösten"

    # game/code/data/replay_room.rpy:5
    old "C09: Josie's First Time"
    new "C09: Josies erstes Mal"

    # game/code/data/replay_room.rpy:5
    old "C09: Josie's First Time (Trans)"
    new "C09: Josies erstes Mal (Trans)"

    # game/code/data/replay_room.rpy:5
    old "C09: Satisfying Ronja"
    new "C09: Ronja zufriedenstellen"

    # game/code/data/replay_room.rpy:5
    old "C09: Penetrating Diana"
    new "C09: Ficke Diana"

    # game/code/data/replay_room.rpy:5
    old "Don't Stop Diana"
    new "Halt Diana nicht auf"

    # game/code/data/replay_room.rpy:5
    old "C09: Exploring With Irene"
    new "C09: Mit Irene auf Entdeckungsreise"

    # game/code/data/replay_room.rpy:5
    old "Investigate the Cult."
    new "Untersuche die Sekte."

    # game/code/data/replay_room.rpy:5
    old "C09: Helping Isa"
    new "C09: Isa helfen"

    # game/code/data/replay_room.rpy:5
    old "Investigate the Winners Club."
    new "Untersuche den Gewinner Club."

    # game/code/data/replay_room.rpy:5
    old "C09: Flirting With Foxy & Lilith"
    new "C09: Flirten mit Foxy & Lilith"

    # game/code/data/replay_room.rpy:5
    old "Investigate the Syndicate."
    new "Untersuche das Syndikat."

    # game/code/data/replay_room.rpy:5
    old "C09: Zemfira's Temptations"
    new "C09: Zemfiras Versuchungen"

    # game/code/data/replay_room.rpy:5
    old "Insult Xasan."
    new "Beleidige Xasan."

    # game/code/data/replay_room.rpy:5
    old "C09: Winner's Strip Tease"
    new "C09: Striptease des Gewinners"

    # game/code/data/replay_room.rpy:5
    old "C09: Naked Mile With Iona & Isa"
    new "C09: Nackte Meile mit Iona & Isa"

    # game/code/data/replay_room.rpy:5
    old "C09: Rope Play With Delphia"
    new "C09: Seilspiel mit Delphia"

    # game/code/data/replay_room.rpy:5
    old "C09: Blowjob From Nia"
    new "C09: Blowjob von Nia"

    # game/code/data/replay_room.rpy:5
    old "C09: Dominating Fleur"
    new "C09: Dominierende Fleur"

    # game/code/data/replay_room.rpy:5
    old "C09: Nia & Silvia Threesome"
    new "C09: Dreier mit Nia & Silvia"

    # game/code/data/replay_room.rpy:5
    old "Investigate respective groups."
    new "Untersuche die jeweiligen Gruppen."

    # game/code/data/replay_room.rpy:5
    old "C09: Silvia & Zemfira Threesome"
    new "C09: Dreier mit Silvia & Zemfira"

    # game/code/data/replay_room.rpy:5
    old "C09: Zemfira & Nia Threesome"
    new "C09: Dreier mit Zemfira & Nia"

