translate chinese strings:

    # game/code/data/achievement_gallery.rpy:5
    old "01: Scratching the Surface"
    new "01: Scratching the Surface"

    # game/code/data/achievement_gallery.rpy:5
    old "Finish the game."
    new "Finish the game."

    # game/code/data/achievement_gallery.rpy:5
    old "02: Dulled Euphoria"
    new "02: Dulled Euphoria"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Iona's path."
    new "Play Iona's path."

    # game/code/data/achievement_gallery.rpy:5
    old "03: Monika's Melons"
    new "03: Monika's Melons"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Monika's path."
    new "Play Monika's path."

    # game/code/data/achievement_gallery.rpy:5
    old "04: Queen Bae"
    new "04: Queen Bae"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Isa's path."
    new "Play Isa's path."

    # game/code/data/achievement_gallery.rpy:5
    old "05: Down With the Bourgeoisie"
    new "05: Down With the Bourgeoisie"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Sam's path."
    new "Play Sam's path."

    # game/code/data/achievement_gallery.rpy:5
    old "06: Blood and Boners"
    new "06: Blood and Boners"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Delphia's path."
    new "Play Delphia's path."

    # game/code/data/achievement_gallery.rpy:5
    old "07: Bagging Bookworms"
    new "07: Bagging Bookworms"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Irene's path."
    new "Play Irene's path."

    # game/code/data/achievement_gallery.rpy:5
    old "08: Innocence Irrelevant"
    new "08: Innocence Irrelevant"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Lumi's path."
    new "Play Lumi's path."

    # game/code/data/achievement_gallery.rpy:5
    old "09: Strokes on Canvas"
    new "09: Strokes on Canvas"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Ezra's path."
    new "Play Ezra's path."

    # game/code/data/achievement_gallery.rpy:5
    old "10: Wine Mom"
    new "10: Wine Mom"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Sally's path."
    new "Play Sally's path."

    # game/code/data/achievement_gallery.rpy:5
    old "11: Fleur de Passion"
    new "11: Fleur de Passion"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Fleur's path."
    new "Play Fleur's path."

    # game/code/data/achievement_gallery.rpy:5
    old "12: Prescription Desires"
    new "12: Prescription Desires"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Asami's path."
    new "Play Asami's path."

    # game/code/data/achievement_gallery.rpy:5
    old "13: What Did the Fox Moan"
    new "13: What Did the Fox Moan"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Foxy's path."
    new "Play Foxy's path."

    # game/code/data/achievement_gallery.rpy:5
    old "14: Mighty Morgan"
    new "14: Mighty Morgan"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Morgan's path."
    new "Play Morgan's path."

    # game/code/data/achievement_gallery.rpy:5
    old "15: Feared Straight"
    new "15: Feared Straight"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Corinne's path."
    new "Play Corinne's path."

    # game/code/data/achievement_gallery.rpy:5
    old "16: Juggling Favors"
    new "16: Juggling Favors"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Nirmala's path."
    new "Play Nirmala's path."

    # game/code/data/achievement_gallery.rpy:5
    old "17: Lifelong Longing"
    new "17: Lifelong Longing"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Josie's path."
    new "Play Josie's path."

    # game/code/data/achievement_gallery.rpy:5
    old "18: And He Was Born"
    new "18: And He Was Born"

    # game/code/data/achievement_gallery.rpy:5
    old "Select your name."
    new "Select your name."

    # game/code/data/achievement_gallery.rpy:5
    old "19: The True Beginning"
    new "19: The True Beginning"

    # game/code/data/achievement_gallery.rpy:5
    old "Reach the end of the prologue."
    new "Reach the end of the prologue."

    # game/code/data/achievement_gallery.rpy:5
    old "20: Saving Private Josie"
    new "20: Saving Private Josie"

    # game/code/data/achievement_gallery.rpy:5
    old "Save Josie from drowning."
    new "Save Josie from drowning."

    # game/code/data/achievement_gallery.rpy:5
    old "21: On to the Tournament"
    new "21: On to the Tournament"

    # game/code/data/achievement_gallery.rpy:5
    old "Finished the Winner's Part 2."
    new "Finished the Winner's Part 2."

    # game/code/data/achievement_gallery.rpy:5
    old "22: On to the Forest"
    new "22: On to the Forest"

    # game/code/data/achievement_gallery.rpy:5
    old "Finished the Cult's Part 2."
    new "Finished the Cult's Part 2."

    # game/code/data/achievement_gallery.rpy:5
    old "23: On to the Compound"
    new "23: On to the Compound"

    # game/code/data/achievement_gallery.rpy:5
    old "Finished the Syndicate's Part 2."
    new "Finished the Syndicate's Part 2."

    # game/code/data/achievement_gallery.rpy:5
    old "24: Injecting the L"
    new "24: Injecting the L"

    # game/code/data/achievement_gallery.rpy:5
    old "Lose the Winner's tournament."
    new "Lose the Winner's tournament."

    # game/code/data/achievement_gallery.rpy:5
    old "25: Heart Shaped Box"
    new "25: Heart Shaped Box"

    # game/code/data/achievement_gallery.rpy:5
    old "Experience spiritual awakening with Nia."
    new "Experience spiritual awakening with Nia."

    # game/code/data/achievement_gallery.rpy:5
    old "26: Kablooey"
    new "26: Kablooey"

    # game/code/data/achievement_gallery.rpy:5
    old "See the Wasp's warehouse explosion."
    new "See the Wasp's warehouse explosion."

    # game/code/data/achievement_gallery.rpy:5
    old "27: Threesome Therapy"
    new "27: Threesome Therapy"

    # game/code/data/achievement_gallery.rpy:5
    old "Have a threesome with Nia and Silvia."
    new "Have a threesome with Nia and Silvia."

    # game/code/data/achievement_gallery.rpy:5
    old "28: Temptations in Ink"
    new "28: Temptations in Ink"

    # game/code/data/achievement_gallery.rpy:5
    old "Have a threesome with Nia and Zemfira."
    new "Have a threesome with Nia and Zemfira."

    # game/code/data/achievement_gallery.rpy:5
    old "29: Rebel Without a Ball"
    new "29: Rebel Without a Ball"

    # game/code/data/achievement_gallery.rpy:5
    old "Have a threesome with Zemfira and Silvia."
    new "Have a threesome with Zemfira and Silvia."

    # game/code/data/achievement_gallery.rpy:5
    old "30: Hacker's Delight"
    new "30: Hacker's Delight"

    # game/code/data/achievement_gallery.rpy:5
    old "Click the Butcher's street background character."
    new "Click the Butcher's street background character."

    # game/code/data/achievement_gallery.rpy:5
    old "31: Meat Lovers"
    new "31: Meat Lovers"

    # game/code/data/achievement_gallery.rpy:5
    old "Win Greg's minigame."
    new "Win Greg's minigame."

    # game/code/data/achievement_gallery.rpy:5
    old "32: Top of the Class"
    new "32: Top of the Class"

    # game/code/data/achievement_gallery.rpy:5
    old "Earn 18+ points with a group."
    new "Earn 18+ points with a group."

    # game/code/data/achievement_gallery.rpy:5
    old "33: Eskimo Sisters"
    new "33: Eskimo Sisters"

    # game/code/data/achievement_gallery.rpy:5
    old "Have sex with Monika and kiss Olesya."
    new "Have sex with Monika and kiss Olesya."

    # game/code/data/achievement_gallery.rpy:5
    old "34: The Truth Will Free You"
    new "34: The Truth Will Free You"

    # game/code/data/achievement_gallery.rpy:5
    old "Kiss Phoebe and tell Fleur and Asami the truth."
    new "Kiss Phoebe and tell Fleur and Asami the truth."

    # game/code/data/achievement_gallery.rpy:5
    old "35: Junior Detective"
    new "35: Junior Detective"

    # game/code/data/achievement_gallery.rpy:5
    old "Unlock a character on the investigation board."
    new "Unlock a character on the investigation board."

    # game/code/data/achievement_gallery.rpy:5
    old "36: Privates Investigator"
    new "36: Privates Investigator"

    # game/code/data/achievement_gallery.rpy:5
    old "Unlock 20 characters on the investigation board."
    new "Unlock 20 characters on the investigation board."

    # game/code/data/achievement_gallery.rpy:5
    old "37: Master Sleuth"
    new "37: Master Sleuth"

    # game/code/data/achievement_gallery.rpy:5
    old "Unlock 40 characters on the investigation board."
    new "Unlock 40 characters on the investigation board."

    # game/code/data/achievement_gallery.rpy:5
    old "38: Passion Detective"
    new "38: Passion Detective"

    # game/code/data/achievement_gallery.rpy:5
    old "Have sex with 10 girls."
    new "Have sex with 10 girls."

    # game/code/data/achievement_gallery.rpy:5
    old "39: Campus Casanova"
    new "39: Campus Casanova"

    # game/code/data/achievement_gallery.rpy:5
    old "Have sex with all the girls."
    new "Have sex with all the girls."

    # game/code/data/achievement_gallery.rpy:5
    old "40: Professor's Pet"
    new "40: Professor's Pet"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Diana and Ronja's paths."
    new "Play Diana and Ronja's paths."

    # game/code/data/achievement_gallery.rpy:5
    old "41: Fetish Locator"
    new "41: Fetish Locator"

    # game/code/data/achievement_gallery.rpy:5
    old "Experience all fetishes at the party."
    new "Experience all fetishes at the party."

    # game/code/data/achievement_gallery.rpy:5
    old "42: Calm and Collected"
    new "42: Calm and Collected"

    # game/code/data/achievement_gallery.rpy:5
    old "Throw Brigitta into the pool."
    new "Throw Brigitta into the pool."

    # game/code/data/achievement_gallery.rpy:5
    old "43: Yellow Balling"
    new "43: Yellow Balling"

    # game/code/data/achievement_gallery.rpy:5
    old "Get blue balled by Nia and Zemfira at the party."
    new "Get blue balled by Nia and Zemfira at the party."

    # game/code/data/achievement_gallery.rpy:5
    old "44: Escaping the Matrix"
    new "44: Escaping the Matrix"

    # game/code/data/achievement_gallery.rpy:5
    old "Click the unclickable option in the nightmare."
    new "Click the unclickable option in the nightmare."

    # game/code/data/achievement_gallery.rpy:5
    old "45: Behind the Scenes"
    new "45: Behind the Scenes"

    # game/code/data/achievement_gallery.rpy:5
    old "Play postcredit scene."
    new "Play postcredit scene."

