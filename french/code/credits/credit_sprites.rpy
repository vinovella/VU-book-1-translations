
translate french strings:

    # game/code/credits/credit_sprites.rpy:24
    old "Story Writing -"
    new "Écriture de l'histoire -"

    # game/code/credits/credit_sprites.rpy:34
    old "Art -"
    new "Art -"

    # game/code/credits/credit_sprites.rpy:52
    old "Programming -"
    new "Programmation -"

    # game/code/credits/credit_sprites.rpy:62
    old "Sound design -"
    new "Conception sonore -"

    # game/code/credits/credit_sprites.rpy:70
    old "Music -"
    new "Musique -"

    # game/code/credits/credit_sprites.rpy:84
    old "Testing, Proofreading\n& Translation -"
    new "Tests, Révision\n& Traduction -"

    # game/code/credits/credit_sprites.rpy:96
    old "Special Thanks -"
    new "Remerciements spéciaux -"

    # game/code/credits/credit_sprites.rpy:105
    old "This game was only\nmade possible with\nall the support we have\nfrom our fans"
    new "Ce jeu n'a été possible\nque grâce à tout le soutien\nque nous avons reçu\nde nos fans"

    # game/code/credits/credit_sprites.rpy:112
    old "Huge thanks to\nall of our fans\nand everyone who\nsupported us and\nhelped to bring\nthis story to life"
    new "Un grand merci à\ntous nos fans\net à tous ceux qui\nnous ont soutenus et\naidés à donner vie\nà cette histoire"

    # game/code/credits/credit_sprites.rpy:122
    old "SKIP"
    new "SAUTER"

    # game/code/credits/credit_sprites.rpy:165
    old "Thank you for playing {color=#d18fe7}Taboo University Book One{/color}!"
    new "Merci d'avoir joué à {color=#d18fe7}Taboo University Book One{/color}!"

   # game/code/credits/credit_sprites.rpy:166
    old "See you soon in {color=#d18fe7}Book Two{/color}!"
    new "À bientôt dans {color=#d18fe7}le Livre Deux{/color} !"

    # game/code/credits/credit_sprites.rpy:168
    old "If you liked the game, then please -\n{a=steam://openurl/https://store.steampowered.com/app/2459350/Taboo_University_Book_One/}Buy Taboo University on Steam{/a}"
    new "Si vous avez aimé le jeu, alors s'il vous plaît -\n{a=steam://openurl/https://store.steampowered.com/app/2459350/Taboo_University_Book_One/}Achetez Taboo University sur Steam{/a}"


