translate spanish strings:

    # game/code/data/ib_descriptions.rpy:2
    old "The group that Klara was involved with. A prestigious and exclusive fraternity of rich and/or famous kids."
    new "El grupo en el que participaba Klara. Una prestigiosa y exclusiva fraternidad de chicos ricos y/o famosos."

    # game/code/data/ib_descriptions.rpy:2
    old "The three girls went missing at one of their private parties."
    new "Las tres chicas desaparecieron en una de sus fiestas privadas."

    # game/code/data/ib_descriptions.rpy:2
    old "A gang that Leia was involved with."
    new "Una pandilla en la que estaba involucrada Leia."

    # game/code/data/ib_descriptions.rpy:2
    old "The gang profits from a betting system that involves 'favors' and illegal fighting."
    new "La pandilla se beneficia de un sistema de apuestas que involucra 'favores' y peleas ilegales."

    # game/code/data/ib_descriptions.rpy:2
    old "The cult that Maria was involved with."
    new "El culto en el que María estaba involucrada."

    # game/code/data/ib_descriptions.rpy:2
    old "The lower ranks of the cult are disguised as a self-improvement group."
    new "Los rangos inferiores del culto se disfrazan como un grupo de superación personal."

    # game/code/data/ib_descriptions.rpy:2
    old "The cult encourages sexual activities and holds breeding rituals."
    new "El culto fomenta las actividades sexuales y lleva a cabo rituales de reproducción."

    # game/code/data/ib_descriptions.rpy:2
    old "The cult tried to take Sally's child away."
    new "El culto trató de llevarse al hijo de Sally."

    # game/code/data/ib_descriptions.rpy:2
    old "Went missing February 15th during the Winner's Club party"
    new "Desapareció el 15 de febrero durante la fiesta del Club de los Ganadores."

    # game/code/data/ib_descriptions.rpy:2
    old "Used to be Nia's Guide."
    new "Solía ​​ser la Guía de Nia."

    # game/code/data/ib_descriptions.rpy:2
    old "Modeled for the artist."
    new "Modeló para el artista."

    # game/code/data/ib_descriptions.rpy:2
    old "Maria was a substantial member of the inner circle in the self-help group."
    new "María era un miembro importante del círculo interno del grupo de autoayuda."

    # game/code/data/ib_descriptions.rpy:2
    old "Was a part of intense {i}rituals{/i} within the self-help group and enjoyed participating in them."
    new "Era parte de intensos {i}rituales{/i} dentro del grupo de autoayuda y disfrutaba participar en ellos."

    # game/code/data/ib_descriptions.rpy:2
    old "Worked as a part-time model and actress"
    new "Trabajó como modelo y actriz a tiempo parcial."

    # game/code/data/ib_descriptions.rpy:2
    old "Leia used to track people's debt to the Syndicate."
    new "Leia solía rastrear la deuda de la gente con el Sindicato."

    # game/code/data/ib_descriptions.rpy:2
    old "Involved with the Winner's Club"
    new "Involucrada con el Club de los Ganadores."

    # game/code/data/ib_descriptions.rpy:2
    old "Got mentoring from Diana Mills."
    new "Obtuvo tutoría de Diana Mills."

    # game/code/data/ib_descriptions.rpy:2
    old "Captain of the volleyball team."
    new "Capitana del equipo de voleibol."

    # game/code/data/ib_descriptions.rpy:2
    old "A friend of Klara."
    new "Amiga de Klara."

    # game/code/data/ib_descriptions.rpy:2
    old "Orphaned. Was involved with a bad crowd."
    new "Huérfana. Estuvo involucrada con gente mala."

    # game/code/data/ib_descriptions.rpy:2
    old "Took over Klara's responsibilities in the volleyball team."
    new "Asumió las responsabilidades de Klara en el equipo de voleibol."

    # game/code/data/ib_descriptions.rpy:2
    old "Simon has asked her out multiple times."
    new "Simón la ha invitado a salir varias veces."

    # game/code/data/ib_descriptions.rpy:2
    old "Does not have enough power within the Winner's Club to allow outsiders to come in or choose new candidates."
    new "No tiene suficiente poder dentro del Club de los Ganadores para permitir que personas externas entren o elijan nuevos candidatos."

    # game/code/data/ib_descriptions.rpy:2
    old "Andrejz and Silvia used to date."
    new "Andrejz y Silvia solían ser pareja."

    # game/code/data/ib_descriptions.rpy:2
    old "Silvia has a past that she's not entirely comfortable discussing. She used to be part of the self-help group and dated Andrejz."
    new "Silvia tiene un pasado del que no se siente del todo cómoda hablando. Antes formaba parte del grupo de autoayuda y salía con Andrejz."

    # game/code/data/ib_descriptions.rpy:2
    old "A friend of Leia."
    new "Amiga de Leia."

    # game/code/data/ib_descriptions.rpy:2
    old "Friends with Simon."
    new "Amiga de Simón."

    # game/code/data/ib_descriptions.rpy:2
    old "Attends the shooting range often."
    new "Asiste al campo de tiro con frecuencia."

    # game/code/data/ib_descriptions.rpy:2
    old "Controlling family."
    new "Familia controladora."

    # game/code/data/ib_descriptions.rpy:2
    old "Took over Leia's responsibilities within a secret PVP community."
    new "Asumió las responsabilidades de Leia dentro de una comunidad secreta de Jugadores contra Jugadores."

    # game/code/data/ib_descriptions.rpy:2
    old "Has high enough clearance to enter the Guns n' Rosette bar through the employee only door."
    new "Tiene altura suficiente para entrar en el bar Guns n' Rosette por la puerta exclusiva para empleados."

    # game/code/data/ib_descriptions.rpy:2
    old "Zemfira is technically a member of the Winner's Club, but prefers to be in her current group."
    new "Zemfira es técnicamente miembro del Club de los Ganadores, pero prefiere estar en su grupo actual."

    # game/code/data/ib_descriptions.rpy:2
    old "Zemfira used to be friends with Brigitta, but had a falling out."
    new "Zemfira solía ser amiga de Brigitta, pero tuvo una pelea."

    # game/code/data/ib_descriptions.rpy:2
    old "A friend of Maria."
    new "Amiga de María."

    # game/code/data/ib_descriptions.rpy:2
    old "Interested in tattoos and art. Dislikes her major."
    new "Le interesan los tatuajes y el arte. No le gusta su carrera."

    # game/code/data/ib_descriptions.rpy:2
    old "Part of a self-help group."
    new "Forma parte del grupo de autoayuda."

    # game/code/data/ib_descriptions.rpy:2
    old "Close friends with Maria."
    new "Amiga íntima de María."

    # game/code/data/ib_descriptions.rpy:2
    old "Has a forceful brother named Xasan."
    new "Tiene un hermano enérgico llamado Xasan."

    # game/code/data/ib_descriptions.rpy:2
    old "Part of an inner circle within the self-help group."
    new "Forma parte de un círculo interno dentro del grupo de autoayuda."

    # game/code/data/ib_descriptions.rpy:2
    old "Good friends with Delphia."
    new "Buena amiga de Delphia."

    # game/code/data/ib_descriptions.rpy:2
    old "Nia seems to have been severely affected by Maria's disappearance."
    new "Nia parece haberse visto gravemente afectada por la desaparición de María."

    # game/code/data/ib_descriptions.rpy:2
    old "My dorm neighbor."
    new "Mi vecina de dormitorio."

    # game/code/data/ib_descriptions.rpy:2
    old "Theater actor. Knows Nia."
    new "Actriz de teatro. Conoce a Nia."

    # game/code/data/ib_descriptions.rpy:2
    old "She is the guide of Nia and me."
    new "Ella es la guía de Nia y mía."

    # game/code/data/ib_descriptions.rpy:2
    old "Has a tattoo that's similar to the logo on the self-help workbooks."
    new "Tiene un tatuaje similar al logo de los libros de autoayuda."

    # game/code/data/ib_descriptions.rpy:2
    old "Friends with Zemfira."
    new "Amiga de Zemfira."

    # game/code/data/ib_descriptions.rpy:2
    old "Simon has asked Silvia out multiple times."
    new "Simón le ha pedido a Silvia salir varias veces."

    # game/code/data/ib_descriptions.rpy:2
    old "Knows my real name."
    new "Sabe mi verdadero nombre."

    # game/code/data/ib_descriptions.rpy:2
    old "My roommate."
    new "Es mi compañera de cuarto."

    # game/code/data/ib_descriptions.rpy:2
    old "Transgender"
    new "Transgénero."

    # game/code/data/ib_descriptions.rpy:2
    old "Josie sleepwalks."
    new "Josie es sonámbula."

    # game/code/data/ib_descriptions.rpy:2
    old "Blind. Best friends with Josie."
    new "Ciega. La mejor amiga de Josie."

    # game/code/data/ib_descriptions.rpy:2
    old "My other roommate."
    new "Es mi otra compañera de cuarto."

    # game/code/data/ib_descriptions.rpy:2
    old "Only person aware of the investigation."
    new "Única persona con conocimiento de la investigación."

    # game/code/data/ib_descriptions.rpy:2
    old "My on-campus contact."
    new "Mi contacto en el campus."

    # game/code/data/ib_descriptions.rpy:2
    old "Mentored Klara."
    new "Tutora de Klara."

    # game/code/data/ib_descriptions.rpy:2
    old "Saw me and Ronja together in our late night tryst."
    new "Nos vio a mí y a Ronja juntos en nuestra cita nocturna."

    # game/code/data/ib_descriptions.rpy:2
    old "Helper at campus library."
    new "Ayudante en la biblioteca del campus."

    # game/code/data/ib_descriptions.rpy:2
    old "A friend of Nia and Maria."
    new "Amiga de Nia y Maria."

    # game/code/data/ib_descriptions.rpy:2
    old "Signs on new members of the self-help group."
    new "Inscribe a los nuevos miembros del grupo de autoayuda."

    # game/code/data/ib_descriptions.rpy:2
    old "Irene is Eliina's Guide."
    new "Irene es la Guía de Eliina."

    # game/code/data/ib_descriptions.rpy:2
    old "Irene is also Lumi's Guide."
    new "Irene es también la Guía de Lumi."

    # game/code/data/ib_descriptions.rpy:2
    old "Irene knows about a secret entrance in the library."
    new "Irene conoce una entrada secreta en la biblioteca."

    # game/code/data/ib_descriptions.rpy:2
    old "On the volleyball team."
    new "En el equipo de voleibol."

    # game/code/data/ib_descriptions.rpy:2
    old "Iona uses cocaine. I wonder where she gets her supply from?"
    new "Iona consume cocaína. Me pregunto de dónde obtiene su suministro."

    # game/code/data/ib_descriptions.rpy:2
    old "Swim and volleyball coach. Mentors Silvia."
    new "Entrenadora de natación y voleibol. Mentora de Silvia."

    # game/code/data/ib_descriptions.rpy:2
    old "Volleyball teammate."
    new "Compañera de equipo de voleibol."

    # game/code/data/ib_descriptions.rpy:2
    old "Theater actor."
    new "Actriz de teatro."

    # game/code/data/ib_descriptions.rpy:2
    old "There seems to be some tension between her and Monika."
    new "Parece haber cierta tensión entre ella y Mónica."

    # game/code/data/ib_descriptions.rpy:2
    old "Olesya is noticeably absent from the party. Need to find out why."
    new "Olesya está notablemente ausente de la fiesta. Necesito averiguar por qué."

    # game/code/data/ib_descriptions.rpy:2
    old "Apparently was almost expelled due to some trouble with \"illicit substances\", at least according to Ronja."
    new "Aparentemente casi fue expulsada debido a algunos problemas con \"sustancias ilícitas\", al menos según Ronja."

    # game/code/data/ib_descriptions.rpy:2
    old "Actively hostile."
    new "Activamente hostil."

    # game/code/data/ib_descriptions.rpy:2
    old "Attends a place called \"Guns n' Rosette\"."
    new "Asiste a un lugar llamado \"Guns n' Rosette\"."

    # game/code/data/ib_descriptions.rpy:2
    old "Fleur has an unknown responsibility."
    new "Fleur tiene una responsabilidad desconocida."

    # game/code/data/ib_descriptions.rpy:2
    old "Fleur and Lilith were childhood friends."
    new "Fleur y Lilith eran amigas de la infancia."

    # game/code/data/ib_descriptions.rpy:2
    old "Fleur is friends with Asami."
    new "Fleur es amiga de Asami."

    # game/code/data/ib_descriptions.rpy:2
    old "Has the clearance to enter the back room of the Guns n' Rosette bar."
    new "Tiene autorización para entrar en la trastienda del bar Guns n' Rosette."

    # game/code/data/ib_descriptions.rpy:2
    old "Fleur is friends and roommates with Eliina."
    new "Fleur es amiga y compañera de cuarto de Eliina."

    # game/code/data/ib_descriptions.rpy:2
    old "Fleur had an explicit stream of Sabrina on her Laptop."
    new "Fleur tenía un stream explícito de Sabrina en su computadora portátil."

    # game/code/data/ib_descriptions.rpy:2
    old "Fleur is managing a server for some sort of streams."
    new "Fleur está administrando un servidor para algún tipo de transmisiones."

    # game/code/data/ib_descriptions.rpy:2
    old "Foxy is higher in ranking than Fleur. Has enough clearance to let people into the fight club."
    new "Foxy está más arriba en el ranking que Fleur. Tiene suficiente autorización para permitir que la gente entre al club de lucha."

    # game/code/data/ib_descriptions.rpy:2
    old "Foxy is the hype-man for the fights."
    new "Foxy es quien promociona las peleas."

    # game/code/data/ib_descriptions.rpy:2
    old "Friends with Fleur and Zemfira."
    new "Amigos de Fleur y Zemfira."

    # game/code/data/ib_descriptions.rpy:2
    old "Has seen me and Phoebe kissing in the park. Potential real identity explosion risk."
    new "Nos ha visto a Phoebe y a mí besándonos en el parque. Riesgo potencial de explosión de su verdadera identidad."

    # game/code/data/ib_descriptions.rpy:2
    old "Asami and Lilith are event coordinators for their group."
    new "Asami y Lilith son las coordinadoras de eventos de su grupo."

    # game/code/data/ib_descriptions.rpy:2
    old "Friends with Zemfira and Fleur."
    new "Amigo de Zemfira y Fleur."

    # game/code/data/ib_descriptions.rpy:2
    old "Was sentenced imprisonment because of physically assaulting someone."
    new "Fue condenada a prisión por agredir físicamente a alguien."

    # game/code/data/ib_descriptions.rpy:2
    old "Morgan and Corinne are half sisters."
    new "Morgan y Corinne son medias hermanas."

    # game/code/data/ib_descriptions.rpy:2
    old "Corinne will exchange information if I help her get in contact with Morgan."
    new "Corinne intercambiará información si la ayudo a ponerse en contacto con Morgan."

    # game/code/data/ib_descriptions.rpy:2
    old "Corinne Bunker was part of the Syndicate."
    new "Corinne Bunker era parte del Sindicato."

    # game/code/data/ib_descriptions.rpy:2
    old "Knows Zemfira."
    new "Conoce a Zemfira."

    # game/code/data/ib_descriptions.rpy:2
    old "Morgan is involved in matches."
    new "Morgan está involucrado en los partidos."

    # game/code/data/ib_descriptions.rpy:2
    old "Morgan knows Nirmala."
    new "Morgan conoce a Nirmala."

    # game/code/data/ib_descriptions.rpy:2
    old "Morgan is a fighter in an illegal fight club."
    new "Morgan es una luchadora en un club de lucha ilegal."

    # game/code/data/ib_descriptions.rpy:2
    old "Corinne has something to do with the amount of debt Morgan's in."
    new "Corinne tiene algo que ver con la cantidad de deuda que tiene Morgan."

    # game/code/data/ib_descriptions.rpy:2
    old "Morgan is willing to help you if you help her with her debt."
    new "Morgan está dispuesta a ayudarte si la ayudas con su deuda."

    # game/code/data/ib_descriptions.rpy:2
    old "Morgan is indebted to an organization called The Syndicate."
    new "Morgan está en deuda con una organización llamada El Sindicato."

    # game/code/data/ib_descriptions.rpy:2
    old "Friends with Morgan. Cori's assault victim."
    new "Amiga de Morgan. La víctima de asalto de Cori."

    # game/code/data/ib_descriptions.rpy:2
    old "Rory is indebted to the enemy gang Cade is involved with."
    new "Rory está en deuda con la pandilla enemiga con la que está involucrado Cade."

    # game/code/data/ib_descriptions.rpy:2
    old "Cade is involved with a gang."
    new "Cade está involucrado con una pandilla."

    # game/code/data/ib_descriptions.rpy:2
    old "Sabrina's ex-boyfriend."
    new "El exnovio de Sabrina."

    # game/code/data/ib_descriptions.rpy:2
    old "Cade is the \"collector\" for a gang."
    new "Cade es el \"recolector\" de una pandilla."

    # game/code/data/ib_descriptions.rpy:2
    old "Cade is the \"collector\" for a gang called The Wasps."
    new "Cade es el \"cobrador\" de una banda llamada Las Avispas."

    # game/code/data/ib_descriptions.rpy:2
    old "Connected to Zemfira. Performs \"favors\"."
    new "Conectada con Zemfira. Realiza \"favores\"."

    # game/code/data/ib_descriptions.rpy:2
    old "Friends with Quentin and Sabrina"
    new "Amiga con Quentin y Sabrina"

    # game/code/data/ib_descriptions.rpy:2
    old "Nirmala performs \"favors\" to pay off her betting debt."
    new "Nirmala realiza \"favores\" para pagar su deuda de apuestas."

    # game/code/data/ib_descriptions.rpy:2
    old "Friends with Quentin and Nirmala."
    new "Amiga de Quentin y Nirmala."

    # game/code/data/ib_descriptions.rpy:2
    old "Sabrina used to be in a gang with her ex, Cade."
    new "Sabrina solía estar en una pandilla con su ex, Cade."

    # game/code/data/ib_descriptions.rpy:2
    old "Friends with Nirmala and Sabrina."
    new "Amiga de Nirmala y Sabrina."

    # game/code/data/ib_descriptions.rpy:2
    old "Friends with Nia. Part of the self-help group."
    new "Amiga de Nia. Parte del grupo de autoayuda."

    # game/code/data/ib_descriptions.rpy:2
    old "Guided by Irene, and a higher rank than Lumi."
    new "Guiada por Irene, y tiene un rango más alto que Lumi."

    # game/code/data/ib_descriptions.rpy:2
    old "Butcher's sister."
    new "La hermana del carnicero."

    # game/code/data/ib_descriptions.rpy:2
    old "Was a part of a local cult."
    new "Era parte de un culto local."

    # game/code/data/ib_descriptions.rpy:2
    old "The father of Sally's child is someone from the cult."
    new "El padre del hijo de Sally es alguien del culto."

    # game/code/data/ib_descriptions.rpy:2
    old "Sally participated in breeding rituals when she was in the cult."
    new "Sally participó en rituales de reproducción cuando estaba en el culto."

    # game/code/data/ib_descriptions.rpy:2
    old "Sally said there's a secret entrance in the library."
    new "Sally dijo que hay una entrada secreta en la biblioteca."

    # game/code/data/ib_descriptions.rpy:2
    old "Owns the butcher shop."
    new "Es dueño de la carnicería."

    # game/code/data/ib_descriptions.rpy:2
    old "Holds seminars for the self-help group."
    new "Realiza seminarios para el grupo de autoayuda."

    # game/code/data/ib_descriptions.rpy:2
    old "Acts defensive and standoffish toward me."
    new "Actúa a la defensiva y distante conmigo."

    # game/code/data/ib_descriptions.rpy:2
    old "Andrejz wants another \"go\" with Ivy."
    new "Andrejz quiere otra \"oportunidad\" con Ivy."

    # game/code/data/ib_descriptions.rpy:2
    old "Mysterious hooded man. Following me."
    new "Un hombre misterioso encapuchado. Me está siguiendo."

    # game/code/data/ib_descriptions.rpy:2
    old "Stalked me when I went to the butchery."
    new "Me acechó cuando fui a la carnicería."

    # game/code/data/ib_descriptions.rpy:2
    old "Nia's controlling brother."
    new "El hermano controlador de Nia."

    # game/code/data/ib_descriptions.rpy:2
    old "Connected to Fleur."
    new "Conectado con Fleur."

    # game/code/data/ib_descriptions.rpy:2
    old "Knows Fleur enough to drop by her room uninvited."
    new "Conoce a Fleur lo suficiente como para pasar por su habitación sin invitación."

    # game/code/data/ib_descriptions.rpy:2
    old "Xasan tracks people's debts to the Syndicate."
    new "Xasan rastrea las deudas de las personas con el Sindicato."

    # game/code/data/ib_descriptions.rpy:2
    old "Painted Maria."
    new "María pintada."

    # game/code/data/ib_descriptions.rpy:2
    old "A {i}lover{/i} of Maria's. Ezra was very intimate with Maria and is lost without her."
    new "Una {i}amante{/i} de María. Ezra era muy íntima con María y está perdida sin ella."

    # game/code/data/ib_descriptions.rpy:2
    old "She told me that Maria was a part of intense {i}rituals{/i} within the self-help group and that she enjoyed participating in them."
    new "Me dijo que María era parte de intensos {i}rituales{/i} dentro del grupo de autoayuda y que disfrutaba participar en ellos."

    # game/code/data/ib_descriptions.rpy:2
    old "Seems to be spying on me."
    new "Parece estar espiándome."

    # game/code/data/ib_descriptions.rpy:2
    old "Was previously involved with the Winner's Club."
    new "Anteriormente estuvo involucrada con el Club de los Ganadores."

    # game/code/data/ib_descriptions.rpy:2
    old "Stalks the Winner's Club."
    new "Acecha el Club de los Ganadores."

    # game/code/data/ib_descriptions.rpy:2
    old "Hates Isa. Wants revenge."
    new "Odia a Isa. Quiere venganza."

    # game/code/data/ib_descriptions.rpy:2
    old "She's been keeping a close eye on Isa."
    new "Ha estado vigilando de cerca a Isa."

    # game/code/data/ib_descriptions.rpy:2
    old "Sam told that members of a rival team are in town and might be planning to spy on the Winner's Club."
    new "Sam dijo que los miembros de un equipo rival están en la ciudad y podrían estar planeando espiar al Club de los Ganadores."

    # game/code/data/ib_descriptions.rpy:2
    old "Sam's got some sort of plan in motion. It's clear she's playing a deep game."
    new "Sam tiene algún tipo de plan en marcha. Está claro que está jugando un juego profundo."

    # game/code/data/ib_descriptions.rpy:2
    old "Sam is trying to seduce Isa. She's not taking no for an answer."
    new "Sam está tratando de seducir a Isa. Ella no acepta un no por respuesta."

    # game/code/data/ib_descriptions.rpy:2
    old "Used to date Isa. Trying to get back together."
    new "Solía ​​salir con Isa. Tratando de volver a estar juntos."

    # game/code/data/ib_descriptions.rpy:2
    old "Does she want Isa or does she want to hurt her?"
    new "¿Quiere a Isa o quiere hacerle daño?"

    # game/code/data/ib_descriptions.rpy:2
    old "Got kicked from the Winner's Club because she crashed an important party that Isa threw."
    new "La echaron del Club de Ganadores porque se coló en una fiesta importante que organizó Isa."

    # game/code/data/ib_descriptions.rpy:2
    old "Sam seems to have connections with the Rival CCC team."
    new "Sam parece tener conexiones con el equipo Rival CCC."

    # game/code/data/ib_descriptions.rpy:2
    old "Sam distrusts Brigitta."
    new "Sam desconfía de Brigitta."

    # game/code/data/ib_descriptions.rpy:2
    old "New member of the self-help group."
    new "Nuevo integrante del grupo de autoayuda."

    # game/code/data/ib_descriptions.rpy:2
    old "I have to convince Lumi to get a tattoo of the self-help group's symbol so I can advance in their ranking."
    new "Tengo que convencer a Lumi de que se tatúe el símbolo del grupo de autoayuda para que pueda ascender en su clasificación."

    # game/code/data/ib_descriptions.rpy:2
    old "Guided by Irene."
    new "Guiada por Irene."

    # game/code/data/ib_descriptions.rpy:2
    old "The self-help group seems to be putting significant pressure on her to get tattooed"
    new "El grupo de autoayuda parece estar ejerciendo una presión significativa sobre ella para que se tatúe."

    # game/code/data/ib_descriptions.rpy:2
    old "Guides or manages a remote community that's a part of the self-help group."
    new "Guía o administra una comunidad remota que forma parte del grupo de autoayuda."

    # game/code/data/ib_descriptions.rpy:2
    old "Member of The Winner's Club."
    new "Miembro del Club de los Ganadores."

    # game/code/data/ib_descriptions.rpy:2
    old "Used to date Sam."
    new "Solía ​​salir con Sam."

    # game/code/data/ib_descriptions.rpy:2
    old "Isa was in part responsible for the party that the missing girls disappeared at."
    new "Isa fue en parte responsable de la fiesta en la que desaparecieron las chicas desaparecidas."

    # game/code/data/ib_descriptions.rpy:2
    old "Isa orchestrated Sam's exile from the Winner's Club."
    new "Isa orquestó el exilio de Sam del Club de los Ganadores."

    # game/code/data/ib_descriptions.rpy:2
    old "Caught spying on our practice."
    new "Atrapado espiando nuestra práctica."

    # game/code/data/ib_descriptions.rpy:2
    old "They ran when confronted. Need to keep an eye out for them in the future."
    new "Corrieron cuando nos enfrentaron. Es necesario estar atento a ellos en el futuro."

    # game/code/data/ib_descriptions.rpy:2
    old "Ronja was in the self-help group."
    new "Ronja estaba en el grupo de autoayuda."

    # game/code/data/ib_descriptions.rpy:2
    old "Apparently Ronja enjoyed Andre's company while in the self-help group."
    new "Aparentemente, Ronja disfrutaba de la compañía de Andre mientras estaba en el grupo de autoayuda."

    # game/code/data/ib_descriptions.rpy:2
    old "Ronja somehow got info about Guns n' Rosette from Fleur."
    new "Ronja de alguna manera consiguió información sobre Guns n' Rosette de Fleur."

    # game/code/data/ib_descriptions.rpy:2
    old "Ronja is married. And also cheating on her husband with me."
    new "Ronja está casada. Y también engaña a su marido conmigo."

    # game/code/data/ib_descriptions.rpy:2
    old "There seems to be some tension between her and Olesya."
    new "Parece haber cierta tensión entre ella y Olesya."

translate spanish strings:

    # game/code/data/ib_descriptions.rpy:2
    old "The Winner's Volleyball team uses drugs to enhance themselves."
    new "El equipo Ganador de Voleibol usa drogas para mejorarse."

    # game/code/data/ib_descriptions.rpy:2
    old "Andrejz, Nerium, Sloan, Yvonne, and Ivy seem to be very high-ranking in their cult."
    new "Andrejz, Nerium, Sloan, Yvonne e Ivy parecen ocupar un rango muy alto en este culto."

    # game/code/data/ib_descriptions.rpy:2
    old "The cult worships a godly figure named Narurra. They have their own intricate lore of how this god came to be."
    new "El culto adora a una figura divina llamada Narurra. Tienen su propia e intrincada historia sobre cómo surgió esta diosa."

    # game/code/data/ib_descriptions.rpy:2
    old "Recently ended a relationship with her boyfriend. Feeling vulnerable and burdened by managerial duties and recent events."
    new "Ha terminado recientemente una relación con su novio. Se siente vulnerable y agobiada por las obligaciones directivas y los acontecimientos recientes."

    # game/code/data/ib_descriptions.rpy:2
    old "Designated as the backup leader in case of emergency during the rescue operation. Indicates trust and responsibility within The Syndicate."
    new "Designada como líder de respaldo en caso de emergencia durante la operación de rescate. Indica confianza y responsabilidad dentro de El Sindicato."

    # game/code/data/ib_descriptions.rpy:2
    old "Nia doesn't see how the self-help group is a cult, she's already too indoctrinated."
    new "Nia no ve al grupo de autoayuda como una secta, ya está demasiado adoctrinada."

    # game/code/data/ib_descriptions.rpy:2
    old "Good friends with Monika."
    new "Un buen amigo de Mónica."

    # game/code/data/ib_descriptions.rpy:2
    old "Simon knows about what I did with Monika at the hotel."
    new "Simón sabe lo que hice con Mónica en el hotel."

    # game/code/data/ib_descriptions.rpy:2
    old "Regularly participates in drug use, familiar with the process."
    new "Participa regularmente en el consumo de drogas, familiarizado con el proceso."

    # game/code/data/ib_descriptions.rpy:2
    old "Irene believes her spiritual scrying hinted at the missing girls' disappearance."
    new "Irene cree que su visión espiritual insinuó la desaparición de las chicas desaparecidas."

    # game/code/data/ib_descriptions.rpy:2
    old "Reluctantly participates in drug use, appears uncomfortable with the situation."
    new "Participa de mala gana en el consumo de drogas, parece incómoda con la situación."

    # game/code/data/ib_descriptions.rpy:2
    old "Iona knows about what I did with Monika at the hotel."
    new "Iona sabe lo que hice con Mónica en el hotel."

    # game/code/data/ib_descriptions.rpy:2
    old "Focused on gaining a strategic advantage in the upcoming tournament against CCC."
    new "Centrada en obtener una ventaja estratégica en el próximo torneo contra CCC."

    # game/code/data/ib_descriptions.rpy:2
    old "Good friends with Simon."
    new "Una buena amiga de Simón."

    # game/code/data/ib_descriptions.rpy:2
    old "Expresses strong dislike and distrust towards Olesya, referring to her as \"murderous\"."
    new "Expresa una fuerte antipatía y desconfianza hacia Olesya, refiriéndose a ella como \"asesina\"."

    # game/code/data/ib_descriptions.rpy:2
    old "Mentioned trouble in the theater group due to an incident involving Olesya."
    new "Mencionó problemas en el grupo de teatro debido a un incidente que involucró a Olesya."

    # game/code/data/ib_descriptions.rpy:2
    old "Participates in the team's drug use without visible discomfort."
    new "Participa en el consumo de drogas del equipo sin molestias visibles."

    # game/code/data/ib_descriptions.rpy:2
    old "Allegedly capitalized on Olesya's vulnerabilities during her addiction struggles, leading to animosity between them."
    new "Supuestamente aprovechó las vulnerabilidades de Olesya durante sus luchas contra la adicción, lo que generó animosidad entre ellas."

    # game/code/data/ib_descriptions.rpy:2
    old "Half-sibling of Olesya."
    new "Media hermana de Olesya."

    # game/code/data/ib_descriptions.rpy:2
    old "Monika describes Olesya as being murderous."
    new "Mónica describe a Olesya como una asesina."

    # game/code/data/ib_descriptions.rpy:2
    old "Shows hesitation and discomfort regarding the team's drug use."
    new "Muestra vacilación e incomodidad respecto al uso de drogas por parte del equipo."

    # game/code/data/ib_descriptions.rpy:2
    old "Olesya's drug use was the catalyst for the team's drug use."
    new "El consumo de drogas de Olesya fue el catalizador del consumo de drogas del equipo."

    # game/code/data/ib_descriptions.rpy:2
    old "Lost a leg in a car accident, which led to an opioid addiction and dependency on performance-enhancing drugs for volleyball."
    new "Perdió una pierna en un accidente automovilístico, lo que le provocó adicción a los opioides y dependencia de drogas para mejorar el rendimiento en el voleibol."

    # game/code/data/ib_descriptions.rpy:2
    old "Half-sibling of Monika"
    new "Media hermana de Mónica"

    # game/code/data/ib_descriptions.rpy:2
    old "Advocates for and administers performance-enhancing drugs to the team, emphasizing a win-at-all-costs mentality."
    new "Defiende y administra al equipo drogas para mejorar el rendimiento, haciendo hincapié en una mentalidad de ganar a toda costa."

    # game/code/data/ib_descriptions.rpy:2
    old "Allegedly responsible for a change in the drug schedule for the volleyball team which might be due to my adverse reaction to the drug."
    new "Presuntamente responsable de un cambio en el calendario de medicamentos del equipo de voleibol que podría deberse a mi reacción adversa al medicamento."

    # game/code/data/ib_descriptions.rpy:2
    old "Expresses strong frustration towards the team's performance and dependency on performance-enhancing drugs."
    new "Expresa una fuerte frustración hacia el desempeño del equipo y su dependencia de las drogas que mejoran el rendimiento."

    # game/code/data/ib_descriptions.rpy:2
    old "Indicates a need to \"report back\" about the loss, suggesting accountability to an external party."
    new "Indica la necesidad de \"informar\" sobre la pérdida, sugiriendo responsabilidad ante una parte externa."

    # game/code/data/ib_descriptions.rpy:2
    old "Brigitta knows about what I did with Monika at the hotel."
    new "Brigitta sabe lo que hice con Mónica en el hotel."

    # game/code/data/ib_descriptions.rpy:2
    old "Reveals a history of violence and survival, indicating a tough upbringing."
    new "Revela una historia de violencia y supervivencia, lo que indica una educación dura."

    # game/code/data/ib_descriptions.rpy:2
    old "Despite her tough exterior, she has a tendency to mediate conflicts, pointing to a deeper sense of responsibility towards others."
    new "A pesar de su exterior duro, tiene tendencia a mediar en los conflictos, lo que indica un sentido más profundo de responsabilidad hacia los demás."

    # game/code/data/ib_descriptions.rpy:2
    old "Reported missing."
    new "Reportada como desaparecida."

    # game/code/data/ib_descriptions.rpy:2
    old "Allegedly kidnapped by Cade and taken to a warehouse controlled by The Wasps."
    new "Supuestamente secuestrada por Cade y llevada a un almacén controlado por Las Avispas."

    # game/code/data/ib_descriptions.rpy:2
    old "Xasan intends to bomb The Wasps."
    new "Xasan tiene la intención de bombardear a Las Avispas."

    # game/code/data/ib_descriptions.rpy:2
    old "Allegedly kidnapped Nirmala, Sabrina, and Q."
    new "Presuntamente secuestró a Nirmala, Sabrina y Q."

    # game/code/data/ib_descriptions.rpy:2
    old "The self-help group seems to be putting significant pressure on her to get tattooed."
    new "El grupo de autoayuda parece estar ejerciendo una presión importante sobre ella para que se tatúe."

    # game/code/data/ib_descriptions.rpy:2
    old "Lumi feels like she was a burden to her parents and hopes that the group will give her the confidence and strength to feel less like a burden."
    new "Lumi siente que era una carga para sus padres y espera que el grupo le dé la confianza y la fuerza para sentirse menos como una carga."

    # game/code/data/ib_descriptions.rpy:2
    old "Lumi has decided to get tattooed."
    new "Lumi ha decidido tatuarse."

    # game/code/data/ib_descriptions.rpy:2
    old "Lives at the campground with Nerium, Yvonne, and Sloan. Manages the team of forest rangers and seems to guide them spiritually."
    new "Vive en el campamento con Nerium, Yvonne y Sloan. Dirige el equipo de guardabosques y parece guiarlos espiritualmente."

    # game/code/data/ib_descriptions.rpy:2
    old "Member of the rival CCC volleyball team."
    new "Miembro del equipo rival de voleibol."

    # game/code/data/ib_descriptions.rpy:2
    old "Tried to spy on us during practice."
    new "Intentó espiarnos durante la práctica."

    # game/code/data/ib_descriptions.rpy:2
    old "Tried to seduce me during the conference."
    new "Intentó seducirme durante la conferencia."

    # game/code/data/ib_descriptions.rpy:2
    old "Engages in manipulative and seductive behavior. Exhibits sadistic traits."
    new "Se involucra en comportamientos manipuladores y seductores. Exhibe rasgos sádicos."

    # game/code/data/ib_descriptions.rpy:2
    old "Hellena wants to strike a deal with the leader of The Syndicate."
    new "Hellena quiere llegar a un acuerdo con el líder de El Sindicato."

    # game/code/data/ib_descriptions.rpy:2
    old "Seen with The Syndicate. Potentially connected."
    new "Se le ha visto con El Sindicato. Potencialmente conectada."

    # game/code/data/ib_descriptions.rpy:2
    old "Captain of the rival CCC volleyball team."
    new "Capitán del equipo rival de voleibol."

    # game/code/data/ib_descriptions.rpy:2
    old "Acknowledges the misconduct of team members but denies involvement. Expresses desire for fair play."
    new "Reconoce la mala conducta de los miembros del equipo pero niega su participación. Expresa su deseo de juego limpio."

    # game/code/data/ib_descriptions.rpy:2
    old "Has known and had previous contact with Klara."
    new "Ha conocido y tenido contacto previo con Klara."

    # game/code/data/ib_descriptions.rpy:2
    old "Found me unconscious and took me to her hotel room."
    new "Me encontró inconsciente y me llevó a su habitación de hotel."

    # game/code/data/ib_descriptions.rpy:2
    old "Assists The Winners with logistics. Helped pick out the hotel for the team's tournament."
    new "Asiste a Los Ganadores con la logística. Ayudó a elegir el hotel para el torneo del equipo."

    # game/code/data/ib_descriptions.rpy:2
    old "Involved in managing press and charity activities for the Winner's Group. Expected the team to attend a press conference."
    new "Involucrada en la gestión de prensa y actividades benéficas para El grupo Los Ganadores. Se esperaba que el equipo asistiera a una conferencia de prensa."

    # game/code/data/ib_descriptions.rpy:2
    old "Visits the campgrounds. Seems to know Andrejz and Ivy."
    new "Visita los campamentos. Parece conocer a Andrejz e Ivy."

    # game/code/data/ib_descriptions.rpy:2
    old "Lives at the campground."
    new "Vive en el campamento."

    # game/code/data/ib_descriptions.rpy:2
    old "Lives at the campground with Nerium and Sloan. Used to be on fire watch duty before she was too pregnant."
    new "Vive en el campamento con Nerium y Sloan. Solía ser guardia de incendios antes de estar demasiado embarazada."

    # game/code/data/ib_descriptions.rpy:2
    old "Lives at the campground with Yvonne and Nerium. Focuses on wildlife conservation."
    new "Vive en el campamento con Yvonne y Nerium. Se centra en la conservación de la vida silvestre."

    # game/code/data/ib_descriptions.rpy:2
    old "Lives at the campground with Yvonne and Sloan. Manages and tracks the local flora."
    new "Vive en el campamento con Yvonne y Sloan. Gestiona y rastrea la flora local."

    # game/code/data/ib_descriptions.rpy:2
    old "A highly revered figure in the Children of Narurra cult."
    new "Una figura muy venerada en el culto de los Hijos de Narurra."

translate spanish strings:

   # game/code/data/ib_descriptions.rpy:2
    old "Bonded with Nia and Zemfira over their missing friends."
    new "Unida a Nia y Zemfira por sus amigas desaparecidas."

    # game/code/data/ib_descriptions.rpy:2
    old "Bonded with Silvia and Nia over their missing friends."
    new "Unida a Silvia y Nia por sus amigas desaparecidas."

    # game/code/data/ib_descriptions.rpy:2
    old "Bonded with Zemfira and Silvia over their missing friends."
    new "Unida a Zemfira y Silvia por sus amigas desaparecidas."

    # game/code/data/ib_descriptions.rpy:2
    old "She is into rope play and watersports."
    new "Le gustan los juegos de cuerdas y los deportes acuáticos."

    # game/code/data/ib_descriptions.rpy:2
    old "Friends with Olesya."
    new "Amigos de Olesya."

    # game/code/data/ib_descriptions.rpy:2
    old "Iona is into voyeurism/public sex."
    new "A Iona le gusta el voyeurismo y el sexo en público."

    # game/code/data/ib_descriptions.rpy:2
    old "Half-sibling of Monika."
    new "Media hermana de Mónica."

    # game/code/data/ib_descriptions.rpy:2
    old "Friends with Josie."
    new "Amiga de Josie."

    # game/code/data/ib_descriptions.rpy:2
    old "Has secret conversations with Prof. Harlan."
    new "Tiene conversaciones secretas con el profesor Harlan."

    # game/code/data/ib_descriptions.rpy:2
    old "Fleur likes to be soft dommed."
    new "A Fleur le gusta que la dominen suavemente."

    # game/code/data/ib_descriptions.rpy:2
    old "Lilith helps Asami with the party planning, and Foxy with managing the fighters."
    new "Lilith ayuda a Asami con la planificación de la fiesta y a Foxy con la gestión de los luchadores."

    # game/code/data/ib_descriptions.rpy:2
    old "The cult is pressuring Eliina to get pregnant."
    new "La secta está presionando a Eliina para que quede embarazada."

    # game/code/data/ib_descriptions.rpy:2
    old "Winner's Club's party host."
    new "Anfitriona de la fiesta del Club de los Ganadores."

    # game/code/data/ib_descriptions.rpy:2
    old "Has a file drawer in the employee section of the fight club."
    new "Tiene un archivador en la sección de empleados del club de lucha."

    # game/code/data/ib_descriptions.rpy:2
    old "Keeps random trash in her file drawer."
    new "Guarda basura al azar en su cajón de archivos."

    # game/code/data/ib_descriptions.rpy:2
    old "Gabbie has possession of Leia's phone."
    new "Gabbie tiene posesión del teléfono de Leia."

    # game/code/data/ib_descriptions.rpy:2
    old "Gabbie had conflict with Leia."
    new "Gabbie tuvo un conflicto con Leia."

    # game/code/data/ib_descriptions.rpy:2
    old "#1 suspect in Leia's disappearance."
    new "Sospechosa número uno en la desaparición de Leia."

    # game/code/data/ib_descriptions.rpy:2
    old "Knows that Brig passes out drugs to the Winners. Seems to encourage it."
    new "Sabe que Brig reparte drogas a los ganadores. Parece alentarlo."

    # game/code/data/ib_descriptions.rpy:2
    old "Something happened with Klara."
    new "Algo pasó con Klara."

    # game/code/data/ib_descriptions.rpy:2
    old "Harlan had conflict with Klara."
    new "Harlan tuvo un conflicto con Klara."

    # game/code/data/ib_descriptions.rpy:2
    old "He is the #1 suspect in Klara's disappearance."
    new "Es el sospechoso número uno de la desaparición de Klara."

    # game/code/data/ib_descriptions.rpy:2
    old "Seems to be a high-ranking member of the cult."
    new "Parece ser un miembro de alto rango de la secta."

    # game/code/data/ib_descriptions.rpy:2
    old "Had a conflict with a crazy ex or something."
    new "Tuvo un conflicto con un ex loco o algo así."

    # game/code/data/ib_descriptions.rpy:2
    old "Kara did not have a crazy ex. She had conflict with Maria."
    new "Kara no tenía un ex loco. Tuvo un conflicto con María."

    # game/code/data/ib_descriptions.rpy:2
    old "Kara is #1 suspect in Maria's disappearance."
    new "Kara es la sospechosa número uno en la desaparición de María."

