translate spanish strings:

    # game/code/data/achievement_gallery.rpy:5
    old "01: Scratching the Surface"
    new "01: Rascando la superficie"

    # game/code/data/achievement_gallery.rpy:5
    old "Finish the game."
    new "Termina el juego."

    # game/code/data/achievement_gallery.rpy:5
    old "02: Dulled Euphoria"
    new "02: Euforia apagada"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Iona's path."
    new "Juega la ruta de Iona."

    # game/code/data/achievement_gallery.rpy:5
    old "03: Monika's Melons"
    new "03: Los melones de Mónica"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Monika's path."
    new "Juega la ruta de Mónica."

    # game/code/data/achievement_gallery.rpy:5
    old "04: Queen Bae"
    new "04: Reina Bae"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Isa's path."
    new "Juega la ruta de Isa."

    # game/code/data/achievement_gallery.rpy:5
    old "05: Down With the Bourgeoisie"
    new "05: Abajo la burguesía"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Sam's path."
    new "Juega la ruta de Sam."

    # game/code/data/achievement_gallery.rpy:5
    old "06: Blood and Boners"
    new "06: Sangre y erecciones"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Delphia's path."
    new "Juega la ruta de Delphia."

    # game/code/data/achievement_gallery.rpy:5
    old "07: Bagging Bookworms"
    new "07: Embolsar ratones de biblioteca"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Irene's path."
    new "Juega la ruta de Irene."

    # game/code/data/achievement_gallery.rpy:5
    old "08: Innocence Irrelevant"
    new "08: La inocencia irrelevante"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Lumi's path."
    new "Juega la ruta de Lumi."

    # game/code/data/achievement_gallery.rpy:5
    old "09: Strokes on Canvas"
    new "09: Trazos sobre lienzo"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Ezra's path."
    new "Juega la ruta de Ezra."

    # game/code/data/achievement_gallery.rpy:5
    old "10: Wine Mom"
    new "10: Mamá del vino"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Sally's path."
    new "Juega la ruta de Sally."

    # game/code/data/achievement_gallery.rpy:5
    old "11: Fleur de Passion"
    new "11: La Pasión de Fleur"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Fleur's path."
    new "Juega la ruta de Fleur."

    # game/code/data/achievement_gallery.rpy:5
    old "12: Prescription Desires"
    new "12: Deseos de prescripción"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Asami's path."
    new "Juega la ruta de Asami."

    # game/code/data/achievement_gallery.rpy:5
    old "13: What Did the Fox Moan"
    new "13: ¿Qué gimió Fox?"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Foxy's path."
    new "Juega la ruta de Foxy."

    # game/code/data/achievement_gallery.rpy:5
    old "14: Mighty Morgan"
    new "14: La poderosa Morgan"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Morgan's path."
    new "Juega el camino de Morgan."

    # game/code/data/achievement_gallery.rpy:5
    old "15: Feared Straight"
    new "15: Heterosexual temido"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Corinne's path."
    new "Juega la ruta de Corinne."

    # game/code/data/achievement_gallery.rpy:5
    old "16: Juggling Favors"
    new "16: Favores de malabarismo"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Nirmala's path."
    new "Juega la ruta de Nirmala."

    # game/code/data/achievement_gallery.rpy:5
    old "17: Lifelong Longing"
    new "17: Anhelo de toda la vida"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Josie's path."
    new "Juega la ruta de Josie."

    # game/code/data/achievement_gallery.rpy:5
    old "18: And He Was Born"
    new "18: Y él nació"

    # game/code/data/achievement_gallery.rpy:5
    old "Select your name."
    new "Selecciona tu nombre."

    # game/code/data/achievement_gallery.rpy:5
    old "19: The True Beginning"
    new "19: El verdadero comienzo"

    # game/code/data/achievement_gallery.rpy:5
    old "Reach the end of the prologue."
    new "Alcanza el final del prólogo."

    # game/code/data/achievement_gallery.rpy:5
    old "20: Saving Private Josie"
    new "20: Salvando al soldado Josie"

    # game/code/data/achievement_gallery.rpy:5
    old "Save Josie from drowning."
    new "Salva a Josie de ahogarse."

    # game/code/data/achievement_gallery.rpy:5
    old "21: On to the Tournament"
    new "21: Hacia el torneo"

    # game/code/data/achievement_gallery.rpy:5
    old "Finished the Winner's Part 2."
    new "Terminaste la Parte 2 de los Ganadores."

    # game/code/data/achievement_gallery.rpy:5
    old "22: On to the Forest"
    new "22: Hacia el bosque"

    # game/code/data/achievement_gallery.rpy:5
    old "Finished the Cult's Part 2."
    new "Terminaste la Parte 2 del Culto."

    # game/code/data/achievement_gallery.rpy:5
    old "23: On to the Compound"
    new "23: Hacia el complejo"

    # game/code/data/achievement_gallery.rpy:5
    old "Finished the Syndicate's Part 2."
    new "Terminaste la Parte 2 del Sindicato."

    # game/code/data/achievement_gallery.rpy:5
    old "24: Injecting the L"
    new "24: Inyectando la L"

    # game/code/data/achievement_gallery.rpy:5
    old "Lose the Winner's tournament."
    new "Pierde el torneo de los Ganadores."

    # game/code/data/achievement_gallery.rpy:5
    old "25: Heart Shaped Box"
    new "25: Caja en forma de corazón"

    # game/code/data/achievement_gallery.rpy:5
    old "Experience spiritual awakening with Nia."
    new "Experimenta el despertar espiritual con Nia."

    # game/code/data/achievement_gallery.rpy:5
    old "26: Kablooey"
    new "26: Kablooey"

    # game/code/data/achievement_gallery.rpy:5
    old "See the Wasp's warehouse explosion."
    new "Vea la explosión del almacén de las Avispas."

    # game/code/data/achievement_gallery.rpy:5
    old "27: Threesome Therapy"
    new "27: Terapia de trío"

    # game/code/data/achievement_gallery.rpy:5
    old "Have a threesome with Nia and Silvia."
    new "Haz un trío con Nia y Silvia."

    # game/code/data/achievement_gallery.rpy:5
    old "28: Temptations in Ink"
    new "28: Tentaciones en tinta"

    # game/code/data/achievement_gallery.rpy:5
    old "Have a threesome with Nia and Zemfira."
    new "Haz un trío con Nia y Zemfira."

    # game/code/data/achievement_gallery.rpy:5
    old "29: Rebel Without a Ball"
    new "29: Rebelde sin pelota"

    # game/code/data/achievement_gallery.rpy:5
    old "Have a threesome with Zemfira and Silvia."
    new "Haz un trío con Zemfira y Silvia."

    # game/code/data/achievement_gallery.rpy:5
    old "30: Hacker's Delight"
    new "30: El deleite del hacker"

    # game/code/data/achievement_gallery.rpy:5
    old "Click the Butcher's street background character."
    new "Haz clic en el personaje de la calle del Carnicero."

    # game/code/data/achievement_gallery.rpy:5
    old "31: Meat Lovers"
    new "31: Amantes de la carne"

    # game/code/data/achievement_gallery.rpy:5
    old "Win Greg's minigame."
    new "Gana el minijuego de Greg."

    # game/code/data/achievement_gallery.rpy:5
    old "32: Top of the Class"
    new "32: El mejor de la clase"

    # game/code/data/achievement_gallery.rpy:5
    old "Earn 18+ points with a group."
    new "Gana más de 18 puntos con un grupo."

    # game/code/data/achievement_gallery.rpy:5
    old "33: Eskimo Sisters"
    new "33: Hermanas esquimales"

    # game/code/data/achievement_gallery.rpy:5
    old "Have sex with Monika and kiss Olesya."
    new "Ten sexo con Mónica y besa a Olesya."

    # game/code/data/achievement_gallery.rpy:5
    old "34: The Truth Will Free You"
    new "34: La verdad te hará libre"

    # game/code/data/achievement_gallery.rpy:5
    old "Kiss Phoebe and tell Fleur and Asami the truth."
    new "Besa a Phoebe y confiesa la verdad."

    # game/code/data/achievement_gallery.rpy:5
    old "35: Junior Detective"
    new "35: Detective Junior"

    # game/code/data/achievement_gallery.rpy:5
    old "Unlock a character on the investigation board."
    new "Desbloquea un personaje."

    # game/code/data/achievement_gallery.rpy:5
    old "36: Privates Investigator"
    new "36: Investigador privado"

    # game/code/data/achievement_gallery.rpy:5
    old "Unlock 20 characters on the investigation board."
    new "Desbloquea 20 personajes."

    # game/code/data/achievement_gallery.rpy:5
    old "37: Master Sleuth"
    new "37: Maestro detective"

    # game/code/data/achievement_gallery.rpy:5
    old "Unlock 40 characters on the investigation board."
    new "Desbloquea 40 personajes."

    # game/code/data/achievement_gallery.rpy:5
    old "38: Passion Detective"
    new "38: Detective apasionado"

    # game/code/data/achievement_gallery.rpy:5
    old "Have sex with 10 girls."
    new "Tener sexo con 10 chicas."

    # game/code/data/achievement_gallery.rpy:5
    old "39: Campus Casanova"
    new "39: Campus Casanova"

    # game/code/data/achievement_gallery.rpy:5
    old "Have sex with all the girls."
    new "Tener sexo con todas las chicas."

    # game/code/data/achievement_gallery.rpy:5
    old "40: Professor's Pet"
    new "40: La mascota del profesor"

    # game/code/data/achievement_gallery.rpy:5
    old "Play Diana and Ronja's paths."
    new "Juega las rutas de Diana y Ronja."

    # game/code/data/achievement_gallery.rpy:5
    old "41: Fetish Locator"
    new "41: Localizador de fetiches"

    # game/code/data/achievement_gallery.rpy:5
    old "Experience all fetishes at the party."
    new "Experimenta todos los fetiches en la fiesta."

    # game/code/data/achievement_gallery.rpy:5
    old "42: Calm and Collected"
    new "42: Tranquilo y sereno"

    # game/code/data/achievement_gallery.rpy:5
    old "Throw Brigitta into the pool."
    new "Arroja a Brigitta a la piscina."

    # game/code/data/achievement_gallery.rpy:5
    old "43: Yellow Balling"
    new "43: Bola amarilla"

    # game/code/data/achievement_gallery.rpy:5
    old "Get blue balled by Nia and Zemfira at the party."
    new "Estás con las bolas azules gracias Nia y Zemfira."

    # game/code/data/achievement_gallery.rpy:5
    old "44: Escaping the Matrix"
    new "44: Escapando de la Matrix"

    # game/code/data/achievement_gallery.rpy:5
    old "Click the unclickable option in the nightmare."
    new "Haz clic en la opción no clicable de la pesadilla."

    # game/code/data/achievement_gallery.rpy:5
    old "45: Behind the Scenes"
    new "45: Detrás de escena"

    # game/code/data/achievement_gallery.rpy:5
    old "Play postcredit scene."
    new "Reproduce la escena postcréditos."

